...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    for (var i=0; i<10; i++){
      ...
      otu_ids_new.push("OTU " + filteredSample.otu_ids[i])
    };

    plotValues["otu_ids_new"] = otu_ids_new.reverse();
    plotValues["otu_ids"] = filteredSample.otu_ids.slice(0,10).reverse();
    ...
    var barData = [{
      x: plotValues.otu_values,
      y: plotValues.otu_ids_new,
      text: plotValues.otu_labels,
      type: "bar",
      orientation: "h"
    }     
    ];
    ...
    var bubbleData = [{
      type: "scatter",
      x: plotValues.otu_ids,
      y: plotValues.otu_values,
      mode:"markers",
      marker: {
        size: plotValues.otu_values,
        sizemode: 'diameter',
        color: plotValues.otu_ids
      },
      text : plotValues.otu_labels
    }
    ];
    ...
    var metadata = data.metadata;
    var wfArray = metadata.filter(sampleObj => sampleObj.id == sample);
    var wfrq = wfArray[0].wfreq;
